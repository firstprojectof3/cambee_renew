...
def _match_major(user_major: Optional[str], target_major: Any) -> bool:
    if _is_all_marker(target_major):
        return True
...
def _match_grade(user_grade: Optional[int], target_grade: Any) -> bool:
    if _is_all_marker(target_grade):
        return True
    if user_grade is None:
        return False
    tg_list = _to_list(target_grade)
    norm: List[int] = []
    for g in tg_list:
        if g is None:
            continue
...
def _filter_single_notice(user, notice) -> Tuple[bool, List[str]]:
    ...
    if not _match_major(user_major, n_major):
        reasons.append("전공 불일치")
    if not _match_grade(user_grade, n_grade):
        reasons.append("학년 불일치")
    if not _match_sn_prefix(user_sn, n_sn):
        reasons.append("학번 앞2자리 불일치")
...
# ---------------- Cache (추가) ----------------
# GptSearchCache 모델 임포트 (분리/통합 상황 모두 대응)
try:
    from app.models.gpt_search_cache import GptSearchCache  # 별도 파일로 존재하는 경우
except Exception:
    try:
        from app.models.models import GptSearchCache  # models.py 안에 있는 경우
    except Exception:
        GptSearchCache = None  # 모델이 아직 없으면 None 처리
...
def get_cached_answer(db: Session, question: str):
    """
    동일 의미(공백/대소문자 무시) 질문이 있으면 1건 반환.
    question_norm 컬럼이 없으므로 SQL 측에서 lower(btrim(...))로 정규화.
    """
    if GptSearchCache is None:
        return None
...
def upsert_cache(
    ...
    """
    같은 의미의 질문이 존재하면 갱신, 없으면 삽입.
    (최소 버전: UNIQUE 키 없이 question 정규화 비교)
    """
    if GptSearchCache is None:
        return None
